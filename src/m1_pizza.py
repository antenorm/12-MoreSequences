...
def run_test_generate_points_on_circle():
    ...
    # ------------------------------------------------------------------
    # Test 2:
    expected = [rg.Point(125, 50), rg.Point(100, 75), rg.Point(75, 50), rg.Point(100, 25)]
    circle = rg.Circle(rg.Point(100, 50), 25)
...
def run_test_draw_points_on_circle():
    ...
    title = 'DRAW_POINTS_ON_CIRCLE, test 4:  35 blue dots.'
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(200, 200), 150)
    draw_points_on_circle(window, circle, 35, 'blue')
    window.close_on_mouse_click()
...
def draw_points_on_circle(window, circle, number_of_points, color):
    ...
    list_points = generate_points_on_circle(circle, number_of_points)
    for k in range(len(list_points)):
        cc = rg.Circle(list_points[k], 10)
        cc.fill_color = color
        cc.attach_to(window)
        point1 = list_points[k]
...
def run_test_pizza():
    ...
    title = 'PIZZA test 4'
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(200, 100), 100)
    circle.outline_thickness = 10
    pizza(window, circle, 50, 'yellow', 5)
    window.close_on_mouse_click()
...
def pizza(window, circle, number_of_slices, color, thickness):
    ...
    for k in range(len(list_points)):
        ln = rg.Line(circle.center, list_points[k])
...
def run_test_polygon():
    ...
    circle.fill_color = 'blue'
    circle.outline_thickness = 3
    polygon(window, circle, 10, 'red', 17)
    window.continue_on_mouse_click()
...
def polygon(window, circle, number_of_segments, color, thickness):
    ...
    for k in range(0, len(list_points)):
        ln = rg.Line(list_points[k - 1], list_points[k])
        ln.thickness = thickness
        ln.color = color
        ln.attach_to(window)
        window.render()
...
def fancy_polygon(window, circle, number_of_lines, hops_to_next_point, color, thickness):
    ...
    circle.attach_to(window)
    window.render()
    lists_points = generate_points_on_circle(circle, number_of_lines)
    end = hops_to_next_point
    for k in range(number_of_lines):
        ln = rg.Line(lists_points[k], lists_points[end])
        ln.arrow = 'last'
        ln.thickness = thickness
        ln.color = color
        ln.attach_to(window)
        window.render()
        if end < (len(lists_points) - 1):
            end += 1
        else:
            end = 0
